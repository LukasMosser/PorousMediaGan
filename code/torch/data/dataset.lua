...
function dataset:__init(...)
   ...
   local extensionList = {'jpg', 'png','JPG','PNG','JPEG', 'ppm', 'PPM', 'bmp', 'BMP', 'hdf5'}
...
local function tableToOutput(self, dataTable, scalarTable)
   ...
   data = torch.Tensor(quantity, self.sampleSize[3], self.sampleSize[3], self.sampleSize[3])
...
function dataset:sample(quantity)
   ...
   local scalarTable = {}
   for i=1,quantity do
      local class = 1 --torch.random(1, #self.classes)
