...
opt = {
   ...
   ndf = 8,               -- #  of discrim filters in first conv layer
...
netG:add(SpatialFullConvolution(nz, ngf * 16, 4, 4, 4))
netG:add(SpatialBatchNormalization(ngf * 16)):add(nn.ReLU(true))
-- state size: (ngf*8) x 4 x 4
netG:add(SpatialFullConvolution(ngf * 16, ngf * 8, 4, 4, 4, 2, 2, 2, 1, 1, 1))
netG:add(SpatialBatchNormalization(ngf * 8)):add(nn.ReLU(true))
-- state size: (ngf*4) x 8 x 8
netG:add(SpatialFullConvolution(ngf * 8, ngf * 4, 4, 4, 4, 2, 2, 2, 1, 1, 1))
netG:add(SpatialBatchNormalization(ngf * 4)):add(nn.ReLU(true))
-- state size: (ngf*2) x 16 x 16
netG:add(SpatialFullConvolution(ngf * 4, ngf*2, 4, 4, 4, 2, 2, 2, 1, 1, 1))
netG:add(SpatialBatchNormalization(ngf*2)):add(nn.ReLU(true))
-- state size: (ngf) x 32 x 32
netG:add(SpatialFullConvolution(ngf * 2, ngf, 4, 4, 4, 2, 2, 2, 1, 1, 1))
netG:add(SpatialBatchNormalization(ngf)):add(nn.ReLU(true))
-- state size: (ngf)
netG:add(SpatialFullConvolution(ngf, nc, 4, 4, 4, 2, 2, 2, 1, 1, 1))
netG:add(nn.Tanh())
...
netD:add(nn.WhiteNoise())
-- input is (nc) x 64 x 64
netD:add(SpatialConvolution(nc, ndf,  4, 4, 4, 2, 2, 2, 1, 1, 1))
netD:add(nn.LeakyReLU(0.2, true))
netD:add(nn.VolumetricDropout(0.2))
-- state size: (ndf) x 32 x 32
netD:add(SpatialConvolution(ndf, ndf * 2,  4, 4, 4, 2, 2, 2, 1, 1, 1))
netD:add(SpatialBatchNormalization(ndf * 2)):add(nn.LeakyReLU(0.2, true))
netD:add(nn.VolumetricDropout(0.2))
-- state size: (ndf*2) x 16 x 16
netD:add(SpatialConvolution(ndf * 2, ndf * 4,  4, 4, 4, 2, 2, 2, 1, 1, 1))
netD:add(SpatialBatchNormalization(ndf * 4)):add(nn.LeakyReLU(0.2, true))
netD:add(nn.VolumetricDropout(0.2))
-- state size: (ndf*4) x 8 x 8
netD:add(SpatialConvolution(ndf * 4, ndf * 8,  4, 4, 4, 2, 2, 2, 1, 1, 1))
netD:add(SpatialBatchNormalization(ndf * 8)):add(nn.LeakyReLU(0.2, true))
netD:add(nn.VolumetricDropout(0.2))
-- state size: (ndf*8) x 4 x 4
netD:add(SpatialConvolution(ndf * 8, ndf * 16, 4, 4, 4, 2, 2, 2, 1, 1, 1))
netD:add(SpatialBatchNormalization(ndf * 16)):add(nn.LeakyReLU(0.2, true))
netD:add(nn.VolumetricDropout(0.2))

netD:add(SpatialConvolution(ndf * 16, 1, 4, 4, 4))
netD:add(nn.Sigmoid())
...
for epoch = 1, opt.niter do
   ...
   netD:get(1).std = math.max(netD:get(1).std-(epoch-1)*0.1/100., 0.)
