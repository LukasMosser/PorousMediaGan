...
noise = torch.Tensor(opt.batchSize, opt.nz, opt.imsize, opt.imsize, opt.imsize)
...
local sample_input = torch.randn(2,100,1,1,1)
if opt.gpu > 0 then
    require 'cunn'
    require 'cudnn'
    net:cuda()
...
--change the name here for your own filename
local myFile = hdf5.open('ketton_128_'..opt.seed ..'.hdf5', 'w')
local tens = nn.utils.recursiveType(images, 'torch.DoubleTensor')
local data = myFile:write('data', tens)
myFile.close()
